...
source = ColumnDataSource(data=dict(ind=[int(-1.*10), int(1.)]))
...
for i, tem in enumerate([s0, s1, s2, s3, s4, s5]):
        ...
        tem.line(Spectra[tuple(source.data['ind'])][0], Spectra[tuple(source.data['ind'])][1+i], 
                 line_width=1, line_alpha=0.75, line_color='red')
...
def callback(attrname, old, new):
        ...
        source.data = dict(ind=[(int(zh*10), int(tage))])
        source.change.emit()
